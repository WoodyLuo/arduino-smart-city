...
public:
    ...
    static bool MQTTConfigur(char* client_id,const char *username,const char *passphrase);
	static bool MQTTConnect(const char* host, uint16_t port);
	static bool subscribe(const char * topic);
	static bool unsubscribe(const char * topic);
	static bool publish(const char * topic,const char *data);
	static int  mqtt_read();
	static String subscribe_data();
    /*
