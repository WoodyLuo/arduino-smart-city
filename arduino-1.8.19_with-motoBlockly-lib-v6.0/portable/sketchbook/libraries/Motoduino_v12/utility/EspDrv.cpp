...
bool EspDrv::startClient(const char* host, uint16_t port, uint8_t sock, uint8_t protMode)
{
	...
	else if (protMode==SSL_MODE)
	{
		...
		ret = sendCmd(F("AT+CIPSTART=%d,\"UDP\",\"%s\",1000,%u,2"), 5000, sock, host, port);
...
bool EspDrv::MQTTConfigur(char* client_id,const char *username,const char *passphrase)
{
	LOGDEBUG(F(">Set configuration of MQTT"));
	int ret = sendCmd(F("AT+MQTTUSERCFG=0,1,\"%s\",\"%s\",\"%s\",0,0,\"\""), 5000,client_id,username,passphrase);

	if (ret==TAG_OK)
	{
		LOGDEBUG(F("Configuration OK"));
		return true;
	}
	LOGDEBUG(F("Failed configuration"));
	return false;
}
bool EspDrv::MQTTConnect(const char* host, uint16_t port)
{
	LOGDEBUG(F(">connect of MQTT server"));
	int ret = sendCmd(F("AT+MQTTCONN=0,\"%s\",%u,0"), 5000,host, port);
	...
	LOGDEBUG(F("Failed connect"));
	return false;
}
bool EspDrv::subscribe(const char * topic)
{
	LOGDEBUG(F(">Subscribe topic"));
	int ret = sendCmd(F("AT+MQTTSUB=0,\"%s\",1"), 5000,topic);

	if (ret==TAG_OK)
	{
		LOGDEBUG(F("Subscribe OK"));
		return true;
	}
	LOGDEBUG(F("Failed Subscribe"));
	return false;
}
bool EspDrv::unsubscribe(const char * topic)
{
	LOGDEBUG(F(">Unsubscribe topic"));
	int ret = sendCmd(F("AT+MQTTUNSUB=0,\"%s\""), 5000,topic);

	if (ret==TAG_OK)
	{
		LOGDEBUG(F("Unsubscribe OK"));
		return true;
	}
	LOGDEBUG(F("Failed Unsubscribe"));
	return false;
}
bool EspDrv::publish(const char * topic,const char *data)
{
	LOGDEBUG(F(">Publish data"));
	int ret = sendCmd(F("AT+MQTTPUB=0,\"%s\",\"%s\",0,0"), 5000,topic,data);

	if (ret==TAG_OK)
	{
		LOGDEBUG(F("Publish OK"));
		return true;
	}
	LOGDEBUG(F("Failed Publish"));
	return false;
}
int EspDrv::mqtt_read()
{	
	int idx=0;
	if (espSerial->available())
	{
