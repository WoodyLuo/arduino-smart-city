...
This is a library for our Monochrome OLEDs based on SSD1306 drivers
  ...
  ------> http://www.moto.com/category/63_98
...
/*********************************************************************
I change the moto SSD1306 to SH1106
...
void moto_SH1106::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
moto_SH1106::moto_SH1106(int8_t SID, int8_t SCLK, int8_t DC, int8_t RST, int8_t CS) : moto_GFX(SH1106_LCDWIDTH, SH1106_LCDHEIGHT) {
  cs = CS;
  rst = RST;
  dc = DC;
  sclk = SCLK;
...
moto_SH1106::moto_SH1106(int8_t reset) :
moto_GFX(SH1106_LCDWIDTH, SH1106_LCDHEIGHT) {
  sclk = dc = cs = sid = -1;
  rst = reset;
}
  

moto_SH1106::~moto_SH1106(void) {
  if (buffer) {
    free(buffer);
    buffer = NULL;
  }
...
void moto_SH1106::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
...
void moto_SH1106::invertDisplay(uint8_t i) {
  if (i) {
...
void moto_SH1106::SH1106_command(uint8_t c) { 
  if (sid != -1)
  {
    // SPI
...
/*void moto_SH1106::startscrollright(uint8_t start, uint8_t stop){
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// display.scrollright(0x00, 0x0F) 
void moto_SH1106::startscrollleft(uint8_t start, uint8_t stop){
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// display.scrollright(0x00, 0x0F) 
void moto_SH1106::startscrolldiagright(uint8_t start, uint8_t stop){
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// display.scrollright(0x00, 0x0F) 
void moto_SH1106::startscrolldiagleft(uint8_t start, uint8_t stop){
  SH1106_command(SH1106_SET_VERTICAL_SCROLL_AREA);  
  SH1106_command(0X00);
  SH1106_command(SH1106_LCDHEIGHT);
...
void moto_SH1106::stopscroll(void){
...
void moto_SH1106::dim(boolean dim) {
  uint8_t contrast;
...
void moto_SH1106::SH1106_data(uint8_t c) {
...
*/
// clear everything
void moto_SH1106::clearDisplay(void) {
  memset(buffer, 0, (SH1106_LCDWIDTH*((SH1106_LCDHEIGHT+7)/8)));
}


inline void moto_SH1106::fastSPIwrite(uint8_t d) {
...
void moto_SH1106::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
  boolean bSwap = false;
  switch(rotation) { 
    case 0:
...
void moto_SH1106::drawFastHLineInternal(int16_t x, int16_t y, int16_t w, uint16_t color) {
...
void moto_SH1106::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
  bool bSwap = false;
  switch(rotation) { 
    case 0:
      break;
...
void moto_SH1106::drawFastVLineInternal(int16_t x, int16_t __y, int16_t __h, uint16_t color) {
